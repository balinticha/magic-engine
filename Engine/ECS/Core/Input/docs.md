...
Instead of checking `if (Keyboard.IsKeyDown(Keys.W))`, game systems check `if (Input.IsPressed(InputAction.MoveUp))`.
This allows for rebindable controls via configuration files.
