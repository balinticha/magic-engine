...
It is designed for games using low-resolution pixel art (e.g., 640x360) that need to be upscaled to modern screens
without jittering or pixel shimmering.
...
The camera handling is hardcoded into the render loop in `MagicGame.cs` to ensure tight integration with the dual-pass
rendering stability.
...
2. **Sub-Pixel Upscaling**:
    ...
    * This allows the entire low-rez image to move smoothly across the screen at high refresh rates, providing fluid
      movement while maintaining pixel-perfect asset alignment relative to each other.
...
Because of the integer snapping and upscaling, converting mouse coordinates to world coordinates requires using the
`GraphicsManager` scaling factors. *See `InputSystem` helper methods if available, or transform standard MouseState
