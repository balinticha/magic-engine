...
Shaders in this engine are consumed by the standard `SpriteBatch`. The vertex processing is handled by the engine, so
your `VertexShader` is usually standard. The main customization happens in the **Pixel Shader**.
...
Shaders are rendered in the texel layer. In the texel layer, the entire RenderTarget is around 640 pixels wide and
300-ish pixels tall.
This is the entire screen. In your RenderBounds component, the defined height and width value IS precisely the amount of
pixels you
can work with for a given effect. This is not something you specifically have to implement in your shader or something
that'd require
...
- `AlphaBlend` mode expects `One, InverseSourceAlpha`. If you don't premultiply, transparency will look "ghostly" or
  wrong (RGB typically stays bright while A drops).
