...
namespace MagicEngine.Engine.Base.Shaders.PostProcessing.Implementations
{
    public class DualFilterBloom : PostProcessStep
    {
        ...
        public override void Apply(SpriteBatch sb, Texture2D source, RenderTarget2D destination)
        {
            ...
            sb.Begin(SpriteSortMode.Immediate, BlendState.Opaque, SamplerState.LinearClamp, DepthStencilState.None,
            ...
            // 2. Draw Bloom
            _effect.CurrentTechnique = _effect.Techniques["Upsample"];
            _effect.Parameters["InverseResolution"]
                ?.SetValue(new Vector2(1.0f / _mips[0].Width, 1.0f / _mips[0].Height));

            // Set actual user Intensity for the final mix
            _effect.Parameters["Intensity"]?.SetValue(Intensity);


            sb.Begin(SpriteSortMode.Immediate, blendState, SamplerState.LinearClamp, DepthStencilState.None,
            ...
            BlendState blendState = null, SamplerState samplerState = null)
        {
            ...
            sb.Begin(SpriteSortMode.Immediate, blendState ?? BlendState.Opaque,
                samplerState ?? SamplerState.LinearClamp, DepthStencilState.None, RasterizerState.CullNone, effect);
