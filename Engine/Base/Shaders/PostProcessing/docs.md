...
The PostProcessing system in MagicEngine allows for applying visual effects to the game's rendered output. It supports a
multi-pass pipeline where effects can be chained together.

The system is managed by `PostProcessingManager` and uses a "Ping-Pong" rendering strategy, swapping between two render
targets (`dest` and `swap`) to apply effects sequentially.
...
* **Use Case**: High-fidelity effects that "break" the pixel grid (e.g., smooth bloom, CRT scanlines, chromatic
  aberration, sub-pixel blur).
...
3. **Implement Apply**:
   ...
   ```
4. **Register it**: Add your step to the `PostProcessingManager.Effects` list (usually in `MagicGame.Initialize` or
   `LoadGameContent`).
